...
module.exports.signup=function(req,res,next){
    var body=req.body;
    if(!body.firstname || !body.lastname || !body.email || !body.mobile || !body.password || !body.gender || !body.username){
        response=responseGenerartor.generate(true,"All fields are mandatory",400,null);
        res.send(response);
    }else if(body.email && body.password && body.gender){
        ...
        if(!validator.isEmail(body.email)){
            ...
            res.send(response);
        }else if(!(body.gender.toLowerCase()=="male" || body.gender.toLowerCase() =="female" || body.gender.toLowerCase() =="others")){
                response=responseGenerartor.generate(true,'Gender should be either Male or Female or Others ',400,null);
                res.send(response);
        }else{
            next();
        }
...
module.exports.signin=function(req,res,next){
    ...
    if(!body.username||!body.password){
         response=responseGenerartor.generate(true,"Username and Password are required",400,null);
        res.send(response);
    }else{
        next();
    }
